...
union float_32 {
 ...
 printf("\tthe base 10 float result is: %g\n", float_32.floating_value_in_32_bits);
 printf("\tthe base 10 int result is: %d\n", float_32.floating_value_as_int); 
 printf("\tcomponents in hex are: %08x\n", float_32.floating_value_as_int);
 printf("\tcomponents in binary are: %s\n", bit_string);
 
 printf("\n\tmatissa:\t0x%x\tor: ", float_32.f_bits.mantissa);
 for (j = 12; j < 40; j++)
 ...
 printf("\n\tsign:\t\t%x\t\tor: %x", float_32.f_bits.sign, float_32.f_bits.sign);
