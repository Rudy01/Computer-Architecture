...
char cstr_6[7], cstr_8[9], cstr_12[13], cstr_16[17], binstr_16[17];
int  label_pc = -1;
FILE *p1;
unsigned short pc = 0;
...
void generate_code(int linum) {
  int line_number, pc, diff, old_pc, sym_val, i;
  char instruction[18], linbuf[10], symbol[26];
...
void update_sym_table(char *symbol) {
  ...
  fprintf(stderr, "errpr from symbol table on line %d\n", pc);
  exit(27);
}
...
int main(int argc, char** argv) {
  int tok, dump_tab = 0, linum = 0, i = 0, object_file = 0;
  unsigned short temp;
